...
/*
    ...
    Super Digital Token (SDT) - BNB reward token
    Author: ARRNAYA SAFU DEV (@ARRN4YA)
    
    100 Billion Total supply
    
    7% tax on Buy 9% on Sell
    
    1% automatically redistributed as BNB, no need to claim
    
    5% marketing on Buy 7% on Sell
    
    1% liquidity

    website: https://www.superdigitaltoken.com
    
    Telegram: https://t.me/SDTLiveChat
...
contract SuperDigitalToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint8 public rewardsFeeOnBuy;
    uint8 public marketingFeeOnBuy;
    uint8 public liquidityFeeOnBuy;
    uint8 public buyFee;

    uint8 public rewardsFeeOnSell;
    uint8 public marketingFeeOnSell;
    uint8 public liquidityFeeOnSell;
    ...
    constructor(address _router) ERC20("Super Digital Token", "SDT") {
        uint8 _rewardsFeeOnBuy = 1;
        uint8 _marketingFeeOnBuy = 5;
        uint8 _liquidityFeeOnBuy = 1;

        uint8 _rewardsFeeOnSell = 1;
        uint8 _marketingFeeOnSell = 7;
        uint8 _liquidityFeeOnSell = 1;
        
        uint256 totalSupply = 100 * 1e9 * 1e18;
        
        address newOwner = address(0x591e77c9a58811366FEB8A9667Cd7F409Bc2dDC9);
        
        // modified to make this easier to update
        maxTxn = totalSupply / 20; // 5%
        swapTokensAtAmount = totalSupply / 10000; // 0.01%
        maxWallet = totalSupply / 20; // 5%

        rewardsFeeOnBuy = _rewardsFeeOnBuy;
        marketingFeeOnBuy = _marketingFeeOnBuy;
        liquidityFeeOnBuy = _liquidityFeeOnBuy;

        buyFee = _rewardsFeeOnBuy + _marketingFeeOnBuy + _liquidityFeeOnBuy;

        rewardsFeeOnSell = _rewardsFeeOnSell;
        marketingFeeOnSell = _marketingFeeOnSell;
        liquidityFeeOnSell = _liquidityFeeOnSell;

        sellFee = _rewardsFeeOnSell + _marketingFeeOnSell + _liquidityFeeOnSell;

        totalFees = buyFee + sellFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(0xD7d3F344fFb4e92eD6165c7c5c28E349072da0eA);
        ...
        */
        ...
        _mint(newOwner, totalSupply); // 100 Billion total supply
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        require(newNum >= totalSupply()/1000, "Cannot lower maxTxn below 0.1%");
        maxTxn = newNum;
    ...
    function updateMaxWallet(uint256 newNum) external onlyOwner {
        require(newNum >= totalSupply()/100, "Can't lower maxWallet");
        maxWallet = newNum;

        emit MaxWalletUpdated(newNum);
    }

    function updateSwapSize(uint val) external onlyOwner {
    ...
    function updateBuyFees(uint8 _marketingFeeOnBuy, uint8 _rewardsFeeOnBuy, uint8 _liquidityFeeOnBuy) external onlyOwner {
        marketingFeeOnBuy = _marketingFeeOnBuy;
        rewardsFeeOnBuy = _rewardsFeeOnBuy;
        liquidityFeeOnBuy = _liquidityFeeOnBuy;
    ...
    function updateMarketingWallet(address newMarketingWallet) external onlyOwner {
        require(newMarketingWallet != address(0), "Can't set to dead address!");
    ...
    function _transfer(
        ...
        if( 
            ...
            if(liquidityFeeOnBuy + liquidityFeeOnSell > 0){
                uint256 swapTokens = contractTokenBalance.mul(liquidityFeeOnBuy + liquidityFeeOnSell).div(totalFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividends = (address(this).balance.sub(initialBalance)).mul(rewardsFeeOnBuy+rewardsFeeOnSell).div(totalFees - (liquidityFeeOnBuy + liquidityFeeOnSell));
    ...
    function withdrawBNB (address _account) external onlyOwner  {
        require (_account != address(0), "$SDT: Can't withdraw to the zero address");
    ...
    function withdrawOtherTokens (address _token, address _account) external onlyOwner {
        require (_token != address(0), "$SDT: Can't withdraw a token of zero address");
        require (_token != address(this), "$SDT: Can't withdraw $SDT");
        require (_account != address(0), "$SDT: Can't withdraw to the zero address");
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("Dividend_Tracker", "Dividend_Tracker") {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 10,000+ tokens
    }
